...
**What you learn:** backend objects, `configure` once, the four ways to hand `simulate()` an agent, and how traces aim the run. **Needs:** `WHILEAI_API_KEY` or `whileai login` for the hosted model, or the provider's key; `simulator=False` with a callable needs nothing. **Takes:** ten minutes.
