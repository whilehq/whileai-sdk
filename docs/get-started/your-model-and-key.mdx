...
| `"vllm:<model>@<url>"` | `OPENAI_API_KEY` or `VLLM_API_KEY`; none for localhost or plain http | `<url>` |
...
## The window, and the key a reasoning model renames

A prompt is cut to fit a context window only where whileai knows the
window: a While-hosted endpoint, or one you name yourself with
`ZP_CONTEXT_TOKENS`. Everywhere else the prompt goes out as you wrote it,
and the server's own 400 is what shrinks it. Before this the `openai:`
path inherited the hosted pool's 4,096 tokens, so a 15,000-character user
turn to a 272k-context model arrived as 7,500 characters with `max_tokens`
lowered from 4096 to 1522, and nothing in the return value said so (#755).

Where the squeeze does run, it is on the record rather than in the margin:

- the reply carries `_prompt_truncated`: the window, the characters asked
  and sent, the reply budget asked and sent;
- the rollout step carries `prompt_truncated` with the same numbers;
- `data.degraded` gains `prompt_truncated`, and `data.warnings` the line
  that names the fix, once per endpoint.

It is reported rather than quietly done because a prompt cut in half moves
an estimate instead of widening its interval: long prompts are not a
random subset, so the rows that lose text are the hard ones.

Every GPT-5 class model, and `o3` and `o4-mini`, rejects `max_tokens` and
asks for `max_completion_tokens`. whileai sends `max_tokens` first, since
every vLLM, Ollama and pre-GPT-5 endpoint takes it, and renames the key
once on a 400 that names the other spelling. There is no table of model
ids to keep current, so the model released next month works too.

## Your own endpoint on Modal

A vLLM you serve yourself on Modal is yours, and `modal.run` in the host
name does not make it ours:

```python
data = wai.simulate("vllm:Qwen/Qwen3-4B@https://mylab--my-qwen-serve.modal.run/v1", tools=TOOLS)
```

The key is `VLLM_API_KEY` or `OPENAI_API_KEY`, the same as any other `vllm:`
endpoint, and no While account is involved. Only the two Modal app prefixes
the platform itself serves from take an account key, and the SDK matches on
those rather than on the suffix. Before this the SDK
matched the bare `modal.run` suffix, so serving your own Qwen3-4B on your
own L40S was answered with "Hosted models need an account key: run `wai
login`" — an error whose own advice was the spelling you had already typed.
Modal is a first-class partner and nothing in the loop requires our hosting.

## A host that is gone

A 404 is not a slow server. An endpoint that was stopped or renamed
answers the same way forever, so the run counts permanent failures (404,
405, 410, 501) per host and stops after three against one host with
nothing succeeding in between, naming the host and the way out. Three
rather than one because a host swapping its route mid-deploy answers a
single 404 and then serves (`DEAD_HOST_STRIKES`: convention, untested). A
5xx, a 429 and a timeout are not counted: those are cold containers and
rate limits, and they keep every retry they had. Before this a run against
a stopped app sat at `0/1000 rollouts` for 7m14s with no error and no
mention of a 404 anywhere (#816).
