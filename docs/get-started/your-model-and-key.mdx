...
and nothing else. Never a prompt, a completion or a request log; Bedrock
invocation logging is off and the endpoint logs errors without content.
Each account gets 600 requests a minute across its keys, answered with a
429 and a `Retry-After` past that.
