...
Two facts about imported models, from the AWS docs: the import takes
merged weights in the Hugging Face layout (a LoRA adapter is merged into
its base first, then uploaded to S3), and an idle import is unloaded, so
the first call after a pause can take minutes and returns
`ModelNotReadyException` until it is back; the SDK retries and then
says so in one sentence. Converse returns no log-probabilities, so
`logprobs=` has no effect on this route.
