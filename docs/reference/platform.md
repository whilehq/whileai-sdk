...
**What a row can hold.** The platform stores a fixed amount of text per field, and the numbers are the API's (`backend/lambda/api/evalrows.js` in the website repo), mirrored once in `whileai.platform`: on every row, `prompt`, `reply`, `reference` and `detail` at 2000 characters (`ROW_TEXT_MAX`) and `why` at 400 (`EXAMPLE_WHY_MAX`); on the card's sample, `prompt` and `reply` at 1200 (`EXAMPLE_TEXT_MAX`); a `rubric` at 4000; 20 examples; 500 rows a call and 20,000 rows behind one score; 8 tags per row, keys up to 32 characters and values up to 80. Text past a cap is not refused: `Example()` cuts the middle out and leaves `[... N chars cut ...]` in its place, keeping the head (the approach) and the tail (the answer), and logs one warning per field that names the cap. A reasoning trace of 7,877 characters posts as its first 988 and last 988 with the count between them; to choose what is kept, shorten the text before `Example()`. The full text stays where the SDK wrote it.
