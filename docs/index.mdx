...
**What you learn:** the loop, simulate to train, as one program. **Needs:** nothing. **Takes:** about twenty seconds on a laptop.
...
grade, measure, select, train, on your machine against your models, no
...
Never trained a model? [Learn](/learn) is nine lessons from zero, each
...
| Train and export | `rows.export`, `export_environment`, `platform.train`, `platform.serve` | Loss masks. A `verifiers` or OpenEnv environment for GRPO. Hosted LoRA SFT, GRPO, DPO, RM. |
