...
batch = [
    {"reward": 1.0, "logprobs": [-0.4, -0.7, -0.2], "behavior_logprobs": [-0.5, -0.7, -0.3]},
...
gradient. That proves the arithmetic is the paper's arithmetic. It does
not prove the papers' results. No GPU replication of any of the three
has been run in this library yet; the numbers in the papers (a 30B model
trained at a lag of eight updates beating GRPO at a lag of one, in
FlashREINFORCE; the reasoning and coding gains in SAO and BPCO) are the
papers' numbers, measured on the papers' setups, and lesson 4's rule
applies to them as to anything else. When a recipe reproduces one under
`recipes/papers/`, with the interval and the noise floor, this page will
say so.
...
is the same loop with a real model on both sides, and
[Train on your own GPU](https://github.com/whilehq/whileai-sdk/tree/main/recipes/04-train)
is where a recipe for these three methods will land when one has run.
