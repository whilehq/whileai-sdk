...
description: "Nine short lessons for an engineer who has never trained a model. Each one runs offline, with no key, in under a minute."
---
...
jump. These nine lessons close that jump one word at a time.
...
<Steps>
  ...
  <Step title="A production trace is one try with a score">
    Real traffic comes one attempt per ask, scored after the fact, in a
    world you cannot replay. The three methods that train on that shape.
    [Read it](/learn/train-on-production-traces).
  </Step>
</Steps>
