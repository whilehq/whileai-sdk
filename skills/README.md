...
| `pick-a-method/` | you have graded rows and must choose SFT, GRPO, DPO, OPSD, OPD or GroupwiseGrading | nothing; it reads the per-task band (floor, saturated, mixed), scores a teacher before OPD, checks the tokenizer, and names the method and the skill that trains it |
