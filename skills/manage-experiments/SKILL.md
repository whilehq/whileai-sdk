...
description: >
  ...
  withwhile.com/platform: the question first, then for every run four
  lines (Changed, Moved, Why, Reproduce), one picture, the rows that
...
They want one look: what changed, did it move, why, and can I run it
...
is the flipped asks, not an adjective. The note has four lines a person
reads in order; the picture is one bar per arm with its interval.
...
Sweeps are the same four lines per arm. Name the arm by the one thing it
...
four lines: Changed, Moved, Why, Reproduce. Under each score, the asks
