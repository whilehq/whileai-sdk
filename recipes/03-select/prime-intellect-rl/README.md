...
library reads. Both roles need a model: the hosted Qwen on your account key
(`wai login`), or your own endpoint named on the call. About three minutes for
800 rollouts. `diagnose.py` and `export_prompts.py` run offline on any graded
row file.
...
```bash
uv add whileai
wai login                    # or name your own endpoint on the call
