...
One field is re-spelled on the way out: a tool call inside `messages` is
the flat `{"name", "arguments": {dict}}` on a rollout row and becomes the
OpenAI wire shape (`{"id", "type": "function", "function": {"name",
"arguments": "<json string>"}}`) in every export, so a reward that reads
tool calls should go through `wai.verify.tool_calls(row)`, which returns
the same `ToolCall` list (name, arguments as a dict, id) from either.
