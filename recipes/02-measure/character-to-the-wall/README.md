...
python run.py --model anthropic:claude-haiku-4-5          # judge defaults to the student

# a different-family judge on any OpenAI-compatible endpoint (recommended)
python run.py --model anthropic:claude-haiku-4-5 \
    --judge-url https://judge/v1 --judge my-judge --judge-key $JKEY

# or run the student on an OpenAI-compatible endpoint too
