...
|---|---|---|
| room to move | does the model already pass nearly everything? | an eval at 0.95 has nowhere to show an improvement; this is benchmark saturation [1] |
...
| self-noise | how far does the number move when nothing changes? | post-training evals move 0.25 to 1.5 points between runs where nothing changed [1]. A claim under twice that is indistinguishable from re-running |
| the judge | did grading succeed, and is reward tracking a cheap feature? | an ungraded eval scores nothing, and a judge that rewards length is measuring length [2] |
| contamination | did the training prompts already contain the eval? | 8-gram overlap, the Tülu 3 method [3] |
...
This is the agentic-evaluation problem stated as a check [1]: every
