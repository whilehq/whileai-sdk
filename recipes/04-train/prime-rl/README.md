...
python run.py --dry-run       # free: write the three configs and stop; no Modal, no key
python run.py --validate      # write the three configs, dry-run each on a CPU container (cents)
python run.py                 # deploy, spawn the three runs (2 H100s each, about 15 min an arm): about $6
