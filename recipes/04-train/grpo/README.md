...
`--seed 0` fixes the data: every run at the same seed builds the same 117
prompts (91 name an order id), the same 92 train and the same 25 holdout,
byte for byte, so two runs that differ only in `--steps` or `--run-name`
are paired on one holdout and their before/after numbers compare. A
different seed is a different prompt set and a different holdout. To
freeze the set beyond the seed, write it once and pass it back in with
...
The template writer gives 117 distinct prompts at `--seed 0` (91 name an
order id, 16 do not, 10 are off topic), so the holdout is 25 prompts and
every pass@1 interval is about a quarter wide. `prompts.jsonl` is a
