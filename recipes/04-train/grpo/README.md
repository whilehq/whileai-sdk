...
uv run sh recipes/04-train/grpo/smoke.sh                            # free: prompts, split, reward on scripted policies; no key, no GPU
modal run recipes/04-train/grpo/train_modal.py --steps 10    # about 3 minutes of A10G, about 5 cents: does the reward curve move at all
modal run recipes/04-train/grpo/train_modal.py               # the run: 40 steps, under 15 minutes, about 30 cents
...
`--seed 0` fixes the data: every run at the same seed builds the same
prompt set and the same holdout, byte for byte, on every Python from 3.10
to 3.14 (119 prompts, 95 naming an order id, 94 train and 25 holdout). Two
runs that differ only in `--steps` or `--run-name` are paired on one
holdout and their before/after numbers compare; a different seed is a
different prompt set and a different holdout. To freeze the set across
machines and SDK versions, write it once and pass it back in with
`--prompts-file` (the model-written `prompts.jsonl` below is one such
file).
...
- **During:** `reward`, `reward_std`, `kl`, `completion_length` and the
  progress bar at while.ai/platform/training, from
  `wai.TrainerCallback`.
...
all, and a second seed on the prompts, since 25 holdout prompts is a wide
...
third decimal. The data is: the same `--seed` writes the same prompt set
and the same holdout on every run. `build_prompts` calls `simulate` with
`reproducible=True`; without it, which situations landed under the budget
depended on thread timing, and two runs at `--seed 0` got 112 and 119
prompts with two different holdouts (whilehq/whileai-sdk#450).

The template writer gives 119 distinct prompts at `--seed 0` on every
Python (95 name an order id, 16 do not, 8 are off topic), so the holdout
is 25 prompts and every pass@1 interval is about a quarter wide. `prompts.jsonl` is a
...
climbing the same way as on the template set. The interval is half the
width it was with 25 holdout prompts, which is the point.
