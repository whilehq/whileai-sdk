...
`--seed 0` fixes the data: every run at the same seed on the same Python
builds the same prompt set and the same holdout, byte for byte (117
prompts, 91 naming an order id, 92 train and 25 holdout on Python 3.12
and later; 118, 92 and 26 on 3.10 and 3.11, where `sum()` adds floats
differently and one near-tie in the writer's novelty filter lands the
other way). Two runs that differ only in `--steps` or `--run-name` are
paired on one holdout and their before/after numbers compare; a different
seed is a different prompt set and a different holdout. To freeze the set
across machines and Python versions, write it once and pass it back in
with `--prompts-file` (the model-written `prompts.jsonl` below is one such
file).
...
The template writer gives 117 distinct prompts at `--seed 0` on Python
3.12 (91 name an order id, 16 do not, 10 are off topic; one more on 3.11),
so the holdout is 25 prompts and every pass@1 interval is about a quarter
wide. `prompts.jsonl` is a
