...
[while.ai/platform](https://while.ai/platform) (the hosted
...
```bash
PYTHONUTF8=1 modal run --detach train_grpo_modal.py --spawn --run-name t2s-r1-check \
  --thinking --skip-eval --steps 10 --gpu H100 --accum 4 --max-completion-length 1536   # about 12 H100 minutes, under $1: does the reward move at all
PYTHONUTF8=1 modal run --detach train_grpo_modal.py --spawn --run-name t2s-r1 \
  --thinking --skip-eval --steps 100 --gpu H100 --accum 4 --max-completion-length 1536  # the round: under two hours, about $8
```

About 65 s a step in thinking mode on an H100 (completions average 900
tokens; $3.95 an hour at Modal's on-demand price), so 100 steps is under
two hours and roughly $8. `--skip-eval`
...
- **Grade the turn you asked for.** `simulate(max_turns=1)` still wrote a
  ...
  low until the rows were cut at the first reply (whilehq/whileai-sdk#586).
  Count assistant turns per row before reading any table.
- **Re-cut the band every round.** A prompt band chosen on the base is
  solved by the end of round 1; round 2 on the same band ran at reward
  0.95 with 60-88% zero-variance groups and moved nothing. Read
  `frac_reward_zero_std` in the first steps; above 0.5, stop and re-cut.
...
| Qwen3.5-9B r1, step 75, second holdout pass (replication) | 0.83 (0.79..0.86) | 0.75 | 0.88 | 0.00 | 0.03 | 0% |
| Qwen3.5-9B r1, step 25, renamed | 0.79 (0.75..0.82) | 0.69 | 0.86 | 0.00 | 0.04 | 0% |
| Qwen3.5-9B r1, step 50, renamed | 0.81 (0.77..0.84) | 0.73 | 0.86 | 0.00 | 0.04 | 0% |
| Qwen3.5-9B r2, step 25, renamed (from r1 step 75; truncation scored 0, 3,072 cap) | 0.83 (0.80..0.86) | 0.77 | 0.88 | 0.00 | 0.03 | 1% |
| Qwen3.5-9B **r3, step 25** (from r1 step 75; band re-cut on step 75: 202 prompts) | 0.84 (0.81..0.87) | 0.77 | 0.88 | 0.00 | 0.03 | 0% |
| Qwen3.5-9B **r3, step 50** (the round's end and result) | **0.84 (0.81..0.87)** | **0.78** | **0.88** | 0.00 | 0.03 | 0% |
...
budget": the engine had written a second user turn on 15% of the 9B rows
and 65% of the 4B rows despite `max_turns=1`, because a reply holding a
"?" (plain-text reasoning does) earned a follow-up, written by the agent
model itself, and the grader scored that second reply
(whilehq/whileai-sdk#586, fixed). Cut each row at its first reply and
re-grade, and the 9B base is 0.60, **+0.053 (95% +0.022..+0.084)** above
Qwen3-4B after round 4, the strongest untrained start on this benchmark;
...
five rounds on the 4B did, from a higher start. A second holdout pass on
the same adapter scored 0.83 (0.79..0.86): +0.221 (+0.194..+0.249) vs the
base, +0.088 (+0.063..+0.113) vs Qwen3-4B r5, and +0.007 (-0.003..+0.018)
vs the first pass, so the verdict is **moved, replicated x2**. The
curve, every point correctly served and paired against the base: step 25
+0.181 (+0.155..+0.208), step 50 +0.204 (+0.178..+0.232), step 75 +0.214,
r2 step 25 +0.224 (+0.196..+0.254). Most of the gain is in the first 25
steps; step 25 to 75 adds +0.033 (+0.017..+0.048), and r2 step 25 vs r1
step 75 is +0.010 (-0.002..+0.023), flat (hard +0.023, +0.002..+0.045).
Round 3, from step 75 on a band re-cut from step 75 itself (202 of 601
prompts at 0.2-0.8, 118 shared with the old band), step 25: 0.84
(0.81..0.87), **+0.019 (+0.005..+0.034) vs step 75**, +0.233
(+0.204..+0.263) vs the base; its first training step ran at reward 0.87
with half the groups carrying a gradient, where round 2 had opened at
0.98 and one sixth. A small move that the interval sees, at a headroom
of 0.04 (pass@4 0.88). Step 50, the round's end: 0.84 (0.81..0.87),
+0.019 (+0.006..+0.033) vs step 75 again, easy +0.015 (+0.002..+0.031)
and hard +0.030 (+0.003..+0.056) both up, pass^4 0.78. The same +0.02
at two checkpoints is the re-cut band's whole effect; the curve is flat
between them, so the 9B is saturated on this benchmark at 0.84 with
pass@4 0.88 as the ceiling this prompt and reply budget allow. The r3
step-50 adapter is `whileai/text-to-sql-shop-qwen3.5-9b-r3-step50-vllm`
(vLLM layout) and its rollouts are `eval-qwen3.5-9b-r3-step50-vllm`.
Round 2 stays stopped: its log showed 60-88% of
