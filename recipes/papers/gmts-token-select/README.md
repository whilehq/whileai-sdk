...
passed as `train_runs=`, would resolve it, and on this evidence the resolved
word would be flat, not moved: the interval excludes zero but on the wrong
side, and `check.py` reserves "moved" for a gain, so a collapse never reads as
a move.
...
| Proxy vs target: `wai.compare(proxy=)` | [5] | `proxy=None`: the training reward *is* the target metric, the same binary check, so there is no proxy to over-optimize. `over_optimized` false |
...
- **A flat verdict and a collapse are not the same news, and the table says the same word for both.** This recipe maps `wai.compare`'s `moved_the_wrong_way` to "flat" (`check.py` itself only rejects intervals that cover zero), so an arm that ends below the untrained base reads as "flat" in the index. That is the correct verdict by the rule and it is worth knowing that the rule compresses it; the Result table above says what actually happened.

Verified 2026-09-18, whileai 0.64, TRL 0.19.1 + PEFT 0.16.0 on torch 2.7.1. Round 1, the numbers above: 45.3 GPU minutes, $1.51 on one L40S; round 2: 45.9 minutes, $1.53. Run page: https://while.ai/platform/training/run_5198278ad1c4d851 (round 2: https://while.ai/platform/training/run_93e11a46757d3f56)

## Artifacts on Hugging Face

| what | repo |
|---|---|
| recipe arm (root, the collapsed run) and `baseline/` | [`while-ai/paper-gmts-token-select-1.5b`](https://huggingface.co/while-ai/paper-gmts-token-select-1.5b) |
