...
6. Eval: pass@1 on the same 120 held-out tasks, 4 samples per task at temperature 0.7, the untrained base three times first for the noise floor, each arm once after training; paired delta with a 95% interval (`wai.compare` with `run_std`, `run_std_runs`; `proxy=` left unset because the training reward is the target).
...
| Check | Source | Result |
|---|---|---|
| Eval noise: the base evaluated 3 times, `eval_variance` run_std | [2] | **run_std 0.0199** from 3 re-runs (0.36, 0.40, 0.38); a delta under **0.121** is noise (`wai.noise_band(run_std, df=2)` = 4.30 x sqrt(2) x 0.0199, t at n - 1 because run_std is an estimate from 3 runs; `wai.compare(run_std=, run_std_runs=3)`) |
...
| Proxy vs target: `wai.compare(proxy=)` | [4] | `proxy=None`, over_optimized **false**: the training reward is the target metric, the same binary check |
...
| Pinned: seed, torch, transformers, peft | [the contract](../README.md#the-contract) | seed 17 for the adapter and the sampler, `--seed 0` for the data split; torch 2.7.1, transformers 4.54.0, peft 0.16.0 |
