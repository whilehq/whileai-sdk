...
Xia et al. 2026 found that unregularized harness evolution memorizes: up to
14.1 points on the split it evolved against, at most 4.7 on five benchmarks
it did not see. Their fix constrains both halves of the loop, and one of the
constraints is a pruner that removes changes that are too small, too
expensive or no longer useful; with it the evolved harness ran on 30% fewer
policy tokens [6]. `--prune` is that pruner for one pick.
