...
| `--prune` | off | after the gate, drop each edit of the pick that buys nothing on train, and gate what is left |
| `--prune-tol` | 0.0 | train pass@1 an edit may cost when removed and still be pruned; 0 keeps any edit whose removal loses score |
...
to answer in one sentence and caps the loop at four turns. Each change is a
named `Edit` (a sentence appended to the baseline's instructions, `Disclosure`
fields, or both), so `--prune` can take it back out. `from_edits` in
`common.py` turns the edits into the prompted loop on a real model, or the
scripted stand-in when the model name starts with `scripted`; `fixes` is the
stand-in's side only, the planted mistakes that come back when the edit is
dropped. A candidate can also call `build` with one instructions string; it
runs and gates the same, and `--prune` skips it.
...
- `out/pruned.json`, `out/pruned_gate.json`, `out/<pick>_pruned.py`: with
  `--prune`, each edit's decision (train pass@1 with and without it, cost
  ratio), the gate on the pruned harness, and the pruned candidate as a
  file to copy into `candidates/`.
...
Xia et al. 2026 ran harness evolution with no constraints on legal
workspace tasks (Claude Opus 4.8 as the policy). It raised the evolve split
from 89.4 to 92.8 and moved three unseen benchmarks from 39.7 to 40.3,
while tokens per trial went from 1.56M to 3.80M. With their constraints on
the proposer and the selector, the evolve split rose less (90.5), the
unseen benchmarks rose more (43.6), and the harness used 2.42M tokens [6].
One of the selector's constraints is structural pruning: a component that
has produced no positive gain over the last few rounds is handed back to
the proposer to delete. `--prune` is a one-shot, leave-one-out version of
that rule for one pick.

It takes the pick's edits out one at a time, in the order the file lists
them, and replays the frozen tasks on the search model. An edit goes when
the train pass@1 without it is no lower than with it (less `--prune-tol`)
and the cost per rollout without it is no higher; an edit that saves tokens
at the same score stays. Drops accumulate, so each test is against the
harness as pruned so far. The decisions read the train split only, so the
holdout still decides: the pruned harness is scored on every model and
faces the same gate as the pick, and its holdout is compared with the
pick's.

```bash
python run.py --dry-run --select --prune
```

```text
prune one_sentence: train 0.67 without it vs 0.92, 1.00x the cost in calls -> kept: removing it loses score or raises cost
prune read_result: train 0.83 without it vs 0.92, 1.00x the cost in calls -> kept: removing it loses score or raises cost
prune no_hidden: train 0.83 without it vs 0.92, 1.00x the cost in calls -> kept: removing it loses score or raises cost
...
...
select: 02_check_result_pruned.py beats the baseline on the holdout and on a held-out model at 1.00x its cost
pruned vs pick on the holdout: +0.00 [+0.00, +0.00] over 6 paired tasks, at 1.00x the pick's cost in calls
...
Offline the two `Disclosure` edits always go: the scripted agent ignores the
turn cap and the retry, so they change nothing it does. That is the pruner
working, not a finding about turn caps. On a real model `max_turns` is
enforced by the prompted loop and can cut tokens, and then the cost rule
keeps it. The checked-in 02 was selected live at 0.40x the baseline's
tokens with both edits in; whether they earned that is what a live
`--prune` run says. The pruner costs one replay per edit on the search model
and one full scoring of the pruned harness.

The paper's other constraints are not built here: an edit budget that
shrinks round by round, a proposer steered toward unexplored edits, and a
critic that rejects benchmark-specific proposals before they are scored.
The day holdout, the held-out model and the matched-cost gate cover part of
what the critic is for, after the fact.
...
6. Xia, P., Han, R., Wang, Z., et al. RRSI: Regularized Recursive Self-Improvement of Agent Harnesses. arXiv:2609.24972, 2026.
