...
**Paper:** Meta-Harness: End-to-End Optimization of Model Harnesses, Yoonho Lee et al., arXiv:2603.28052, 2026. https://arxiv.org/abs/2603.28052
**Book:** Lambert 2025, chapter *Evaluation* [2]: the split that decides, the interval over tasks [3], and the re-run band a delta clears before it is a difference.
**Claim:** the harness around a model -- the prompt, the tools, the turn cap, the retry -- is worth optimizing end to end, and a search whose proposer reads every prior candidate's source, score and worst traces, with nothing compressed, finds harnesses that beat the best hand-engineered baselines at fewer context tokens.
**The change:** the object under search is harness code, one Python file per candidate, and a pick has to clear the baseline on held-out tasks, on a held-out model, and at no more cost per rollout before it counts.
...
1. Models: the search model runs every candidate, and at least one more model is held out of the proposer's view entirely. The run in the Result section used Claude Haiku 4.5 as the search model and gpt-4.1-mini held out, both through OpenRouter. `--dry-run` swaps both for seeded scripted stand-ins, so the loop runs offline with no key.
2. Tasks: one frozen set, drawn once by the offline situation writer from the seeds in `common.py` and saved to `out/tasks.jsonl`; every candidate, model and re-run replays it with `tasks=`. `--holdout 0.5` splits it, the train half picks and the holdout half decides. With `--traces`, the frozen set is the agent's own traffic and the holdout is its latest whole days.
3. Judge: a program in `common.py`, not a model. It reads the reply for filler, checks that a reply claiming success sits on a tool result that succeeded, and checks that nothing privileged leaked.
4. Candidates: `candidates/00_baseline.py` is the baseline; every later file is one change, written by the proposer from `out/proposal.md` (every prior candidate's source, score with interval, and five worst train rows). The three checked-in files stand in for the proposer in the dry run.
5. The gate: the pick is the candidate leading the most train tasks, per task the best pass rate with ties shared [4]. It is a result only if the same harness beats the baseline on the held-out tasks and on the held-out model, with an interval over tasks excluding zero on both, at no more than `--cost-margin` above the baseline's cost per rollout [5].
6. Noise floor: the baseline is re-run on the same frozen holdout, and `eval_variance` turns that spread into the band a one-run-per-side delta has to clear.

There is no trained arm anywhere in this recipe: nothing updates a weight, so `checks.train_seeds` in `results.json` is `null` and the seed rule in `recipes/papers/check.py` has nothing to count. The science half of that check applies in full.
...
| Round | What changed | Holdout pass@1, baseline -> pick | vs the baseline |
|---|---|---|---|
| 1 | the loop offline: two scripted models, the three checked-in candidates, 24 asks | 0.67 -> 0.94 | +0.27 [+0.15, +0.42] over 12 paired tasks; the planted rates make these numbers, so the round shows the gate working and replicates nothing |
| 2 | the same three candidates live: Haiku 4.5 as the search model, gpt-4.1-mini held out, 60 asks split 30/30, one round | 0.62 -> 0.99 | +0.38 [+0.27, +0.47] over 30 paired tasks, and +0.09 [+0.04, +0.15] on the held-out model |

**Round 1 found a bug in the judge, not in the harness.** The first version of
`common.judge` counted a text-only turn as a failed tool result and flagged
every "has been processed" as a false success claim. The fix is one
`continue`, and the dry-run numbers did not move, because the scripted agent
never emits a text-only turn -- which is exactly why a scripted round cannot
stand in for a live one.

**Round 2 is not called `moved`, and the reason is the fourth criterion.**
Held to `CONSTITUTION.md`'s repeatable-science row, the run clears three of
the four: the interval excludes zero on both models, the noise floor is four
baseline draws on the frozen holdout (`run_std` 0.038, band 0.17 at df=3), and
the holdout is stronger than the bar (split by seed, a held-out model, and
8-gram decontamination on the `--traces` path). The fourth is a
proxy-vs-target verdict, and this recipe does not measure one: the attribution
it prints is harness versus model, which is a different question.
`checks.over_optimized` is therefore `null`, `recipes/papers/check.py` refuses
`moved` without it, and the verdict in `results.json` is `unresolved` (#809).

**The next round is not a fourth candidate.** This set's ceiling is 0.99, so
the useful next run is a harder one: more seeds in `common.py`, a larger
`--budget`, or the agent's own traffic through `--traces`. A proxy-vs-target
measurement on the search's own selection signal would resolve the verdict.
