...
| Check | Source | Result |
|---|---|---|
| Eval noise: the base evaluated 10 times, `eval_variance` run_std | [4] | **run_std 0.0078 from 10 re-runs**: a delta under 0.025 (`wai.noise_band(run_std, df=9)` = 2.26 x sqrt(2) x run_std, the band on a difference of two re-run draws, t because run_std is an estimate) is noise. The measured +0.067 clears it. On the original 3 re-runs (run_std 0.0169) the honest band was 0.103 and it did not |
...
| Proxy vs target: `wai.compare(proxy=)` | [1] | **not over-optimized.** `marker:shaped_reward` -0.035 [-0.080, +0.009] while pass@1 +0.067: the target moved and the proxy did not follow it up |
