...
python recipe.py --train-seeds 17,18,19   # three seeds an arm, the delta with a between-seed term
```
...
| Pinned: seed, torch, transformers, trl, peft, vllm | [the contract](../README.md#the-contract) | `--train-seeds 17` in the trainer (a comma list runs several), `--seed 0` for the data split; torch 2.13.0, transformers 5.17.0, trl 1.13.0, peft 0.21.0, vllm 0.29.0, whileai 0.110 |
...
- Next: DAPO's dynamic sampling [3] on the lenient arm, whose groups saturate at a 0.57 mean reward.
  Three seeds an arm is now `--train-seeds 17,18,19`: every seed's rows go to `delta_report(train_runs=)`,
  which adds the between-seed term the single-seed verdict was missing.

Verified 2026-09-18, whileai 0.83, TRL 1.13.0 GRPOTrainer + vLLM 0.29.0 on Modal. Run page: https://withwhile.com/platform/training/run_064f9185fdf88eef (recipe arm), https://withwhile.com/platform/training/run_42f97ae9878c4621 (baseline arm); Runs page: https://while.ai/platform/runs?agent=zero-rl-qwen3.5-4b
