...
`0.04` line below. The 2026-09-20 uploads 1.0 and 1.3 to 1.9 were the 0.100
to 0.109 releases under the wrong numbers; they are yanked.
...
- Recipe `recipes/01-simulate/smol-data-envs/`: SmolDataEnvs (FineEnvs, 5,394 Kaggle data-analysis
  ...
  any agent spec. Claude Haiku 4.5, first 24 eval tasks, k=4: pass@1 0.48 [0.30, 0.66] (0.45 on a
  second run), 6 of 24 groups split for GRPO.
...
- `select(rows).export(path)` writes the system prompt and tool schemas whichever
  ...
  `select` no longer ships a tool-calling SFT file whose prompts never show the tools.
